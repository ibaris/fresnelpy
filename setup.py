...
def get_version():
    ...
    with open("fresnelpy/__version__.py") as fp:
...
sources = glob('fresnelpy/**/*/*.{0}'.format(ext))  # Define source directories.

for item in sources:
    ext_modules.append(Extension(item.split('.')[0].replace(os.path.sep, '.'),
                                 [item],
                                 include_dirs=['fresnelpy/bin', os.path.dirname(item), '.']))

if USING_CYTHON:
    cmdclass.update({'build_ext': build_ext})

setup(name='fresnelpy',
      author="Ismail Baris",
      maintainer='Ismail Baris',
      version=get_version(),
      description='Fresnel reflection for different boundaries.',
      ...
      # ~ license='APACHE 2',
      url='https://github.com/ibaris/fresnelpy',
